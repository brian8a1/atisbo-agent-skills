...
2. Choose one capture path:
   - Customer statement, bug, observation, or metric anomaly: call `atisbo_capture` with `mode: signal`.
   - Interview transcript, research notes, product documentation, or reusable context: call `atisbo_capture` with `mode: knowledge` and the appropriate `capture_mode`.
   - External query or metric that supports a Living Document: resolve the document with `atisbo_lookup`, then call `atisbo_capture` with `mode: add_artefact_evidence`.
